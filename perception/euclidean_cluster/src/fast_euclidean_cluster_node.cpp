...
#include "fast_euclidean_cluster_node.hpp"
...
: Node("fast_euclidean_node", options)
{
  // initialize debug tool
  {
    ...
    debug_publisher_ptr_ = std::make_unique<DebugPublisher>(this, "fast_euclidean_cluster_node");
  ...
  cluster_ = std::make_shared<FastEuclideanCluster>(
    use_height, min_cluster_size, max_cluster_size, tolerance);

  using std::placeholders::_1;
  pointcloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    "input", rclcpp::SensorDataQoS().keep_last(1),
    std::bind(&FastEuclideanClusterNode::onPointCloud, this, _1));
...
RCLCPP_COMPONENTS_REGISTER_NODE(euclidean_cluster::FastEuclideanClusterNode)
