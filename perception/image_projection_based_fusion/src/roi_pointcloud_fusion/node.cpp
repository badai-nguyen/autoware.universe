...
void RoiPointCloudFusionNode::preprocess(__attribute__((unused))  sensor_msgs::msg::PointCloud2 & pointcloud_msg)
{
  return;
}

void RoiPointCloudFusionNode::postprocess(__attribute__((unused))  sensor_msgs::msg::PointCloud2 & pointcloud_msg)
{
  return;
}
    ...
    __attribute__((unused))  sensor_msgs::msg::PointCloud2 & output_pointcloud_msg)
{
  ...
  projection << camera_info.p.at(0),camera_info.p.at(1),camera_info.p.at(2),camera_info.p.at(3),camera_info.p.at(4),
  camera_info.p.at(5),camera_info.p.at(6),camera_info.p.at(7),camera_info.p.at(8),camera_info.p.at(9),camera_info.p.at(10),
  camera_info.p.at(11);
  geometry_msgs::msg::TransformStamped transform_stamped;
  // transform pointcloud from frame id to camera optical frame id
  {
    const auto transform_stamped_optional = getTransformStamped(
      tf_buffer_, input_roi_msg.header.frame_id, input_pointcloud_msg.header.frame_id, input_roi_msg.header.stamp);
  ...
  for (std::size_t i = 0; i < clusters.size(); ++i) {
    ...
    auto refine_cluster = closest_cluster(cluster, cluster_threshold_radius_, cluster_threshold_distance_);
    ...
    ros_pc_cluster.header = input_pointcloud_msg.header;
